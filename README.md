...
### フロント・バックの分離
フロントエンドとバックエンドを分離した構成を採用し、責務を明確に分けています。
バックエンドはRailsをAPIモードで構築し、「どのようなデータを配信すべきか」を意識した設計にしています。
フロントエンドはNext.jsを採用し、APIから取得したデータを動的に描画することで、SPAのような滑らかな操作感を実現しています。

### 保守性を意識した設計
- 診断ロジックは最初app/servicesにまとめていましたが、名前だけでは何を担う場所か曖昧になりやすかったため、models/diagnosisという名前空間に移動し、責務ごとに整理しました。各クラスは単一責任の原則に従い、1つの処理だけを担うように設計しています。
- usersテーブルにユーザー情報を集約するのではなく、OAuth認証やゲストログインなど認証方式が増えてもusersテーブルの責務が肥大化しないよう、認証情報をuser_credentialsへ分離しました。

### 質問ページ
回答を選択すると自動的に次の質問へスクロールするUIにし、あるカテゴリーの質問にすべて回答すると次のカテゴリーへ進めるようにしています。
20問程度というボリュームでも、ユーザーがストレスなく直感的に回答を進められることを意識しました。

### テスト
RSpecを用いて、ModelSpec・RequestSpecを中心にテストを整備しています。
バリデーションやAPIレスポンスの形が期待通りかを、手動確認に頼らずコードで継続的に検証できるようにしています。

### CI（継続的インテグレーション）構築
GitHub Actionsを用いて、pushのたびに以下が自動実行されるようにしています。

- RuboCop（静的解析）
- RSpec（テスト）
- Biome（Lint・Format）

これにより、レビュー前の段階でコード品質やバグを機械的にチェックできる状態を作り、安心して変更を取り込めるようにしています。
